...
**[[TeamMatch]]**: 단체전 7경기(단식 6+복식 1). 연맹별 방식 다양화 지원 계획(4단1복/5단식/5단2복). **2026-04-14 세트 점수 검증 추가**: 프론트엔드+서버 이중 검증 — 탁구 11점제/듀스 규칙 위반 차단, 서버에서 `set_scores` 기반 세트 승수 재계산(프론트엔드 값 불신뢰).
